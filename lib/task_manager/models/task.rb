...
module TaskManager
    module Models
        class Task < ActiveRecord::Base
            ...
            def <=>(other_task)
                return 0 unless other_task.is_a?(Task)
                
                # First compare by completion status (incomplete tasks first)
                status_comparison = (status == 'completed' ? 1 : 0) <=> (other_task.status == 'completed' ? 1 : 0)
                ...
                overdue_comparison = (overdue? ? 1 : 0) <=> (other_task.overdue? ? 1 : 0)
                ...
                priority_comparison = (priority_order[priority] || 3) <=> (priority_order[other_task.priority] || 3)
